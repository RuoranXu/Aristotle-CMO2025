...
/- 
It has a mistake that \[\lim_k \frac{R(k+1,k)}{R(k,k)}> 1+c.\], c is not equal to 0; I need c is bigger than 0, and find c.
you need: Introducing \ (R) (k, k \) of exponential lower bound (Erd ő s probability method: \( (1+o(1))\frac{k}{\sqrt{2}e}2^{k/2} \) or upper bound \( k^{-c\log k/(\log\log k)} \binom{2k}{k} \) find the c.
-/
